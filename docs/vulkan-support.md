...
`gpu_malloc<T>()` returns `GpuAllocation<T>`, and `gpu_free(allocation)` releases it through a
reference to the unchanged aggregate without a separate `Device*`. Every public operation that needs a Vulkan
...
`write_texture_descriptor()` accepts a live `Device*`, a caller-selected CPU destination, a live `Texture*`,
`TextureDescriptorType::sampled` or `storage`, and an optional `TextureDescriptorDesc`. It writes one
device-sized texture descriptor directly at that address and validates that the texture belongs to
the supplied device. The descriptor's type follows `TextureType`, and
